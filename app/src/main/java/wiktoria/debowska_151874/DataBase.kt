...
class DataBase {
    ...
    constructor(title: String?, org_title: String?, year_pub: Int, rank_pos: Int, pic: String?, photo: String?, error: String?, exp: Int) {
        this.title = title
        this.org_title = org_title
        this.year_pub = year_pub
        this.rank_pos = rank_pos
...
class DataBaseMain(context: Context, name: String?, factory: SQLiteDatabase.CursorFactory?, version: Int): SQLiteOpenHelper(context, DATABASE_NAME, factory, DATABASE_VERSION) {
    companion object {
    ...
    fun findRecord(title: String?): DataBase? {
        ...
        if (cursor.moveToFirst()) {
            ...
            rec = DataBase(id, title, org_title, year_pub, rank_pos, thumb,photos,errors, exp)
            cursor.close()
        }
    ...
    fun findRecord(id: Int): DataBase? {
        val query =
    ...
    fun returnError():String{
        var ans=""
        val query =
            "SELECT $COLUMN_MESSAGE FROM $TABLE_RECORDS WHERE $COLUMN_EXPANSION !=null"
        val db = this.writableDatabase
        val cursor = db.rawQuery(query, null)
        if (cursor.moveToFirst()) {
    ...
    fun getVals(addOns: Int, ord: Orders = Orders._ID, desc: Boolean = false):List<DataBase>{
        val mList: MutableList<DataBase> = ArrayList()
        ...
        do {
            ...
            val photos = cursor.getStringOrNull(6)
            val errors = cursor.getStringOrNull(7)
            rec = DataBase(id, title, org_title, year_pub, rank_pos, thumb,photos,errors, addOns)
            mList.add(rec)
        }while(cursor.moveToNext())
        cursor.close()
        db.close()
        val res: List<DataBase> = mList
        return res
    }
    ...
    fun updateAddOn(id: String, exp: Int) {
        ...
        values.put(COLUMN_IMAGE, rec.photo)
        values.put(COLUMN_MESSAGE, rec.error)
